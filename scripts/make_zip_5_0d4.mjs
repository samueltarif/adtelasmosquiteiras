...
const gitModifiedFiles = gitStatusOutput
  .split('\n')
  .map(l => l.trim())
  .filter(l => l.length > 0 && !l.endsWith('.zip') && !l.endsWith('.sha256') && !l.endsWith('.sha256.txt') && !l.includes('make_zip') && !l.includes('package_'))
...
console.log('1. Verificando existência de todos os arquivos para o pacote da Fase 5.0D.4...')
...
const zipPathDocs = 'docs/phase_5_0d4_delta_external_review.zip'
const zipPathRoot = 'phase_5_0d4_delta_external_review.zip'
const shaPathDocs = 'docs/phase_5_0d4_delta_external_review.zip.sha256'
const shaPathRoot = 'phase_5_0d4_delta_external_review.zip.sha256'
...
fs.writeFileSync(shaPathDocs, `${sha256}  phase_5_0d4_delta_external_review.zip\n`)
fs.writeFileSync(shaPathRoot, `${sha256}  phase_5_0d4_delta_external_review.zip\n`)
