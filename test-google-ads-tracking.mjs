...
console.log('\n--- GRUPO 2: Conversão no API Success (Single Source via GTM) ---')
gtagCalls.length = 0
...
assert.strictEqual(gtagCalls.length, 0, '2.2 Zero direct gtag conversion calls (delegated to GTM via lead_form_success)')

assert.strictEqual(dataLayerPushes.length, 1, '2.3 Exactly 1 dataLayer push')
assert.strictEqual(dataLayerPushes[0].event, 'lead_form_success', '2.4 Canonical event is lead_form_success')
assert.strictEqual(dataLayerPushes[0].submission_id, sub1, '2.5 dataLayer has correct submission_id')

// Garantir que a aplicação não contenha referências ativas à conta antiga ou rótulo
const rawFormConversionCode = fs.readFileSync(path.resolve('app/utils/formConversion.js'), 'utf8')
const rawGtagClientCode = fs.readFileSync(path.resolve('app/plugins/gtag.client.js'), 'utf8')
assert(!rawFormConversionCode.includes('AW-17981093809'), '2.6 formConversion has zero AW-17981093809 references')
assert(!rawFormConversionCode.includes('4GwPCPCPWSjoccELHvhv5C'), '2.7 formConversion has zero 4GwPCPCPWSjoccELHvhv5C references')
assert(!rawFormConversionCode.includes('\'conversion\''), '2.8 formConversion does not call conversion directly')
assert(!rawGtagClientCode.includes('AW-17981093809'), '2.9 gtag.client.js has zero AW-17981093809 config')
console.log('  ✓ 2.1 - 2.9 API Success triggers exactly 1 canonical dataLayer event (lead_form_success) and 0 direct Google Ads calls: PASS')
...
assert.strictEqual(gtagCalls.length, 0, '3.2 No direct gtag calls on duplicate (0)')
...
assert.strictEqual(gtagCalls.length, 0, '4.2 Zero direct gtag calls on second lead')
...
console.log('  ✓ 4.1 - 4.4 New submission_id properly triggers new dataLayer lead_form_success: PASS')
