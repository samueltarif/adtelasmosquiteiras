    ...
    <section class="py-12">
      <div class="container mx-auto px-4 max-w-6xl">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <h2 class="text-2xl font-bold text-[#22345F] mb-4">Atendemos {{ bairro }} e Região</h2>
            <p class="text-gray-600 mb-6">Somos especialistas em instalação de telas mosquiteiras e redes de proteção em {{ bairro }}. Atendimento rápido, orçamento grátis e instalação profissional.</p>
            <div class="space-y-4 mb-8">
              <div class="flex items-start gap-3"><Icon name="lucide:check-circle" class="w-6 h-6 text-[#25D366] flex-shrink-0 mt-1" /><div><h3 class="font-bold text-[#22345F]">Instalação Rápida</h3><p class="text-sm text-gray-600">Instalamos em até 48 horas após aprovação</p></div></div>
          ...
          <div class="bg-white rounded-2xl shadow-xl border-2 border-[#E5EDF8] p-6 md:p-8">
            ...
            <form v-else @submit.prevent="submitForm" class="space-y-4">
              ...
              <select v-model="formData.servico" required class="w-full px-4 py-3 border-2 border-[#E5EDF8] rounded-xl focus:border-[#F49A1A] focus:outline-none"><option value="">Selecione o serviço *</option><option v-for="s in servicosOptions" :key="s" :value="s">{{ s }}</option></select>
              <textarea v-model="formData.mensagem" rows="3" placeholder="Mensagem (opcional)" class="w-full px-4 py-3 border-2 border-[#E5EDF8] rounded-xl focus:border-[#F49A1A] focus:outline-none resize-none"></textarea>
              <button type="submit" :disabled="isSubmitting" class="w-full px-6 py-4 bg-[#F49A1A] hover:bg-[#e08910] text-white rounded-xl font-bold transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"><Icon v-if="!isSubmitting" name="lucide:send" class="w-5 h-5" /><svg v-else class="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>{{ isSubmitting ? 'Enviando...' : 'Enviar Solicitação' }}</button>
            </form>
          </div>
        </div>
      </div>
