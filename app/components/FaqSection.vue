    ...
    <div class="max-w-[1200px] mx-auto px-5">
      ...
      <div class="max-w-[800px] mx-auto">
        <div 
          ...
          :class="[
            'bg-surface p-5 md:p-6 mb-4 rounded-xl border border-border cursor-pointer transition-all duration-300 hover:border-primary hover:shadow-md'
          ]"
        >
          <div class="flex justify-between items-center">
            <h4 class="text-base md:text-lg text-text-primary font-semibold pr-4">{{ faq.question }}</h4>
            <span 
              :class="[
                'text-primary text-2xl font-bold transition-transform duration-300',
          ...
          <div 
            v-show="isOpen(index)"
            class="mt-5 pt-5 border-t border-border text-text-secondary leading-relaxed text-sm md:text-base"
          >
            {{ faq.answer }}
          </div>
        </div>
