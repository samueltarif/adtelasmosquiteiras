...
<template>
  ...
  <header class="md:hidden fixed top-0 left-0 right-0 z-40 px-3 py-2">
    <div
      class="flex items-center justify-between bg-white rounded-full shadow-lg px-3 h-14 transition-all duration-300"
      :class="isScrolled ? 'shadow-xl' : 'shadow-md'"
    >
      <!-- Logo -->
      <button @click="goToHome" class="flex-shrink-0 hover:opacity-80 transition-opacity">
        <img
          src="/images/logo_adt_telas_nova.png"
          alt="AD Telas"
          class="h-9 w-auto"
      ...
      <nav class="flex items-center gap-1 overflow-x-auto scrollbar-hide mx-2 flex-1">
        <button
          v-for="item in menuItems.filter(i => ['hero','problems','reviews','/contato'].includes(i.id))"
          :key="item.id"
          @click="scrollToSection(item)"
          class="flex-shrink-0 px-2 py-1 text-xs font-medium text-gray-700 hover:text-[#22345F] relative whitespace-nowrap"
          :class="item.id === 'hero' ? 'text-[#22345F] font-bold after:absolute after:bottom-0 after:left-0 after:right-0 after:h-0.5 after:bg-[#F49A1A] after:rounded-full' : ''"
        >
          {{ item.label }}
        </button>
      </nav>
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <a
          ...
          class="flex items-center justify-center w-9 h-9 bg-[#25D366] hover:bg-[#20B858] text-white rounded-full transition-all active:scale-95 shadow-sm"
        >
        ...
        <button
          @click="toggleMobileMenu"
          class="flex items-center justify-center w-8 h-8 text-gray-700 hover:text-[#22345F] transition-colors"
        >
